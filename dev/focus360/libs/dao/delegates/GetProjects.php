<?php
/**
 * @version 1.0
 * @created 08-Jun-2009 3:16:47 PM
 */
class GetProjects
{

	static $dataValidator;

	function __construct()
	{
	}



	/**
	 * 
	 * @param user
	 */
	static function execute()
	{
		if( Cache::getInstance()->get( PROJECT_LIST_CACHE_KEY ) != false ){
			//error_log("updating user in cache", 0);
			return Cache::getInstance()->get( PROJECT_LIST_CACHE_KEY );
		}
		// Instantiate the AmazonSDB class
		$sdb = new AmazonSDB();
		// Store the name of the domain
		$domain = PROJECTS_DOMAIN;
		$results = array();
		$results["projects"] = array();
		// Read the domain
		$project_domain = $sdb->domain_metadata( $domain );
		if ($project_domain->isOK())
		{
			//create the query
			$sql = "select appID, name, type, client, createdOn, modifiedOn, description from " . PROJECTS_DOMAIN;
			$response = $sdb->select( $sql );
			if ($response->isOK())
			{
				$response = json_decode( json_encode( $response ) );
			}else{
				return AwsUtils::parseError( $response );
			}
		}else{
			return AwsUtils::parseError( $user_domain );
		}
		if( is_array( $response->{'body'}->{'SelectResult'}->{'Item'} ) ){
			foreach( $response->{'body'}->{'SelectResult'}->{'Item'} as $projectObj ){
				$project = new Project();
				foreach( $projectObj->{'Attribute'} as $itemObj ){
					$project->{ $itemObj->{'Name'} } = $itemObj->{'Value'};
				}
				array_push( $results["projects"], $project );
			}
		}else{
			$project = new Project();
			if( $response->{'body'}->{'SelectResult'}->{'Item'}->{'Attribute'} == null ){
				return new CustomAWSError( 1010, "No projects found." );
			}
			foreach( $response->{'body'}->{'SelectResult'}->{'Item'}->{'Attribute'} as $itemObj ){
				$project->{ $itemObj->{'Name'} } = $itemObj->{'Value'};
			}
			array_push( $results["projects"], $project );
		}
		if( Cache::getInstance()->get( PROJECT_LIST_CACHE_KEY ) != false ){
			//error_log("updating user in cache", 0);
			Cache::getInstance()->replace( PROJECT_LIST_CACHE_KEY, $results, false, Cache::$defaultTimeToLive );
		}else{
			//error_log("adding user in cache", 0);
			Cache::getInstance()->add(PROJECT_LIST_CACHE_KEY, $results, false, Cache::$defaultTimeToLive);
		}
		return $results;
	}

}
?>